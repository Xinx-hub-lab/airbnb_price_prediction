...
## backup preprocessing functions for random forest
def train_clean_2_(df):
    ...
    ## target encoding
    glob_med_neigh, smooth_neigh, df_clean['neighbourhood_cleansed'] = utils.target_encoding2(df_clean, 'price', 'neighbourhood_cleansed')
    glob_med_prope, smooth_prope, df_clean['property_type'] = utils.target_encoding2(df_clean, 'price', 'property_type')
    ## impute 4 from name
    ...
    return df_clean, median_values, bound_values, smooth_neigh, smooth_prope, glob_med_neigh, glob_med_prope
...
def test_clean_2_(df, median_dict, bound_dict, smooth_neigh, smooth_prope, glob_med_neigh, glob_med_prope, test = True):
    ...
    for col in cat_cols:
        df_clean[col] = utils.label_encoding(df_clean[col])
        df_clean[col] = utils.convert_to_category(df_clean[col])
    df_clean['neighbourhood_cleansed'] = df_clean['neighbourhood_cleansed'].map(smooth_neigh).fillna(glob_med_neigh)
    df_clean['property_type'] = df_clean['property_type'].map(smooth_prope).fillna(glob_med_prope)
